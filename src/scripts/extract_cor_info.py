...
def extract_cor_info(pdf_path):
    ...
    student_id_match = re.search(r"\b\d{4}-\s*\d{4,5}-[A-Z]{2}-\d\b", text)
    student_id = student_id_match.group(0).replace(" ", "") if student_id_match else None
    ...
    section_match = re.search(r"SECTION:\s*([A-Z0-9]+)", text)
    section = section_match.group(1) if section_match else None

    pattern = r"([A-Z]{4,5}\s\d{3})\s+(.+?)\s+(BSIT\s[\dA-Z-]+)\s+(.+)"
